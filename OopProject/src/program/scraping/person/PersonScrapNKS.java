...
public class PersonScrapNKS extends PersonScrap{
	...
	public void scrap() throws IOException {
		for (int i = 0; i <= 1455; i += 5) {
			...
			for (int k = 0; k < persons.size(); k++) {
				...
				PersonScrap.list.add(person);
				System.out.println(person.getId() + " - " + person.getName());

				try {
					getPersonImg("https://nguoikesu.com/" + persons.get(k).select("img").first().attr("data-src"),
