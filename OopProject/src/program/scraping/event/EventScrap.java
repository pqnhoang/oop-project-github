package program.scraping.event;
...
public class EventScrap extends WebScrapping implements ScrapException {

	public EventScrap() {
		super("https://nguoikesu.com", "src/app/data/json", "src/app/data/img/event");
	}

	private void getEventImg(String src_image, String name, String dir) {
		try {
			URL url;
			if (!src_image.equals("")) {
		...
		try {
			...
			for (int i = 0; i <= endIndex; i += 5) {
				...
				for (int j = 0; j < elms.size(); ++j) {
					...
					// Luu anh cua su kien
					getEventImg(imgUrl, index + ".png", getImgStoreUrls());
	...
	public void saveToJsonFile(List<Event> list) {
		Gson gson = new Gson();
